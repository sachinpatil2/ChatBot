$(document).ready(function () {
    ...
    location1 = 'http://10.44.122.45:8083/lima'
  ...
  //  $("#UpdatePasswordForm, #welcome-wrapper, #home-wrapper").hide();
    $("#UpdatePasswordForm, #welcome-wrapper").hide();
    $("#home-wrapper").show();
      ...
      /*  username = $("#username").val();
        ...
        function initialCall() {
            ...
            $.post(location1 + "/bot", {
                    ...
                    console.log("inside intial call.."+data);
                    currentTime();
                    systemUtteranceContent = '<li class="mar-btm systemUtterance"> <div class="media-body pad-hor"> <div class="speech"> <p id="systemUtterance" class="more">' + msg + ', ' + data + '</p><p class="speech-time"><i class="fa fa-clock-o fa-fw"></i>' + currentTime() + '</p> </div></div></li>';
                    $('#loading').hide();
	...
	/*------------------------ Custom show read more/less Start --------------------*/
            ...
            var questionData = [];
			$.get('js/autocomplete.json',function(data){
				//console.log("autocomplete json"+JSON.stringify(data));
				for(var i=0;i<data.length;i++){
					questionData.push(data[i].question);
				}
				//console.log("autocomplete json questionData"+JSON.stringify(questionData));
				//questionData = data;
			});
			...
			/*
			// extract last two words to match
			...
			// type ahead plgin initialized
			   $('.userUtterance').typeahead({
	...
	/*------------------------ Custom show read more/less End --------------------*/
